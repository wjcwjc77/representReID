...
class ResNet50(nn.Module):
    def __init__(self, num_classes, loss={'softmax'}, aligned=False, **kwargs):
        ...
        self.base = nn.Sequential(*list(resnet50.children())[:-2])#将原来的resnet50的两层去掉，因为最后两层是平均池化层，和全连接层（全连接层的类别数是1000，所以我们要去掉，然后定义自己的全连接层）
    ...
    def forward(self, x):
        ...
        x = F.avg_pool2d(x, x.size()[2:])  #我们自己定义池化，池化的大小就是x经过base 后的 宽高。[32,2048,8,4] ->[32,2048,1,1]
        f = x.view(x.size(0), -1) #[32,2048,1,1] ->[32,2048]
